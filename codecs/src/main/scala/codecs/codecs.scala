...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = {
   // Decoder.fromPartialFunction { case Json.Num(n) => if ((n).isValidInt) n.intValue else 0 }
    Decoder.fromFunction { case Json.Num(n) => if ((n).isValidInt) Option(n.intValue) else None }
  }

  /** A decoder for `String` values */
  // TODO Define an implicit value of type `Decoder[String]`
  implicit val stringDecoder: Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(n) => n }

  /** A decoder for `Boolean` values */
  // TODO Define an implicit value of type `Decoder[Boolean]`
  implicit val booleanDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(n) => n }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      //???
      case Json.Arr(xs) => Option(xs.map(x=>x.decodeAs(decoder).get))
    }


  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
  Decoder.fromFunction{
    case Json.Obj(map)  => map.get(name).get.decodeAs(decoder)
  }
...
trait ContactsCodecs {
  ...
  implicit def contactEncoder(implicit enc:Encoder[Person]): Encoder[Contacts] = {

   // Encoder.listEncoder(enc).transform[Contacts](x=>x.people) // this works

    ObjectEncoder.field[List[Person]]("people")
      .transform[Contacts](x=>x.people)
  }



  implicit def contactDecoder(implicit dec:Decoder[Person]): Decoder[Contacts] = Decoder.field[List[Person]]("people").transform[Contacts](x=>Contacts(x))
//    Decoder.listDecoder(dec).transform[Contacts](x=>Contacts(x)) // this works
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /*val maybeJsonString = parseJson(""" true """)
    val maybeJsonStringl = parseJson(""" [100,200,300,400] """)
    val maybeJsonObj    = parseJson(""" { "name": "Alice", "age": 42 } """)
    val maybeJsonObj3    = parseJson(""" { "name": "Alice", "age": 42,"married":false } """)
...
//     println(maybeJsonString.flatMap(_.decodeAs[String]))
     println(maybeJsonString.flatMap(_.decodeAs[Boolean]))
     println(maybeJsonObj)
     println(maybeJsonStringl)
     println(maybeJsonObj3)
     //println(maybeJsonStringl.flatMap(_.decodeAs))
    ...
    println(encoder.encode(contacts))
    println(contacts)
    println(encoder.encode(contacts).decodeAs[Contacts].get)
